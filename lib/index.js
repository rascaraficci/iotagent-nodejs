...
class IoTAgent {
  ...
  on(subject, event, callback) {
    logger.debug(`Registering a new callback for subject ${subject} and event ${event}...`);
    this.messenger.on(subject, event, callback);
    logger.debug(`... callback registered`);
  }

  generateDeviceCreateEventForActiveDevices() {
    logger.debug(`Generating device creation events for all current active devices...`);
    this.messenger.generateDeviceCreateEventForActiveDevices();
    logger.debug(`... device creation events for active devices were generated.`);
  }
  
  getTenants() {
    logger.debug(`Retrieving tenants...`);
    var ret = dojotModule.Auth.getTenants(dojotConfig.auth.host);
    logger.debug(`... tenants retrieval was initiated.`);
    return ret;
  }
