...
var defaultConfig = dojotModule.Config;
...
class IoTAgent {
  ...
  constructor(config) {
    ...
    this.config.kafka.consumer["group.id"] = process.env.KAFKA_GROUP_ID || `iotagent-${uuid4()}`;
    this.messenger = new dojotModule.Messenger("iotagent", this.config);
  }
  ...
  init() {
    ...
    return this.messenger.init().then(() => {
      ...
      this.messenger.createChannel(this.config.dojot.subjects.deviceData, "w");
      ...
      this.messenger.on(this.config.dojot.subjects.devices, "message", (tenant, msg) => {
        let parsed = null;
        try {
  ...
  getTenants() {
    ...
    var ret = dojotModule.Auth.getTenants(this.config.auth.url);
  ...
  getDevice(deviceid, tenant) {
    return Promise
      .resolve()
      .then(() => {
        logger.info(`Getting device from ${this.config.deviceManager.url}/device/${deviceid}`, TAG);
        let url = `${this.config.deviceManager.url}/device/${deviceid}`;
  ...
  updateAttrs(deviceid, tenant, attrs, metadata, key = null) {
    ...
    this.messenger.publish(this.config.dojot.subjects.deviceData, tenant, msg, key);
  }
