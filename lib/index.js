...
class IoTAgent {
  ...
  parseConfig(config) {
    ...
    this.devm = getValues(DEVICE_MANAGER_DEFAULTS, config['device-manager'], ['subject', 'manager']);
    this.auth = getValues(TENANCY_MANAGER_DEFAULTS, config['auth'], ['subject', 'manager']);
    this.iota = getValues(IOTAGENT_DEFAULTS, config['iota'], ['subject']);
