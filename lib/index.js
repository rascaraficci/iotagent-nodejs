...
class IoTAgent {
  ...
  initConsumer(retry=true) {
    ...
    let consumer = new kafka.Consumer('internal', TENANCY_MANAGER_DEFAULTS.subject);

    consumer.connectConsumer().then(() => {
  ...
  checkCompleteMetaFields(deviceid, tenant, metadata) {
      if (!metadata) {
        return Promise.reject(new TypeError('Device metadata must be passed to the function'));
      }
      ...
      return Promise
        ...
        .then(() => {
          if (!metadata.hasOwnProperty('templates')) {
            return this.getDevice(deviceid, tenant);
          }
        })
        .then(info => {
          if (info) {
   ...
   */
  updateAttrs(deviceid, tenant, attrs, metadata, use_metadata = false) {
